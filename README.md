...
- **품질 기준 기반 Supervisor**
  - C1~C5 기준(기업당 출처 ≥3, 긍·부정 쿼리 커버, SWOT 4요소 채움, Reference 형식, SUMMARY로 시작해 REFERENCE로 끝나는 구조)을 자동 검증하고,  
...
- **임진영**: 프로젝트 전반 기획, 문제 정의, 멀티 에이전트 아키텍처 및 Graph 설계  
- **백강민**: Supervisor Agent 설계 및 구현, RAG 파이프라인·벡터스토어(pgvector/Hybrid) 구성, LangGraph 워크플로 구체화   
- **민지홍**: LG/CATL/시장/전략/보고서 에이전트 전반 구현, 보고서 자동 생성 파이프라인 개발
