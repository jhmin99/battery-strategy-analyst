...
class MarketAssessmentAgent:
    ...
    def run(self, state: WorkflowState) -> WorkflowState:
        ...
        # 프롬프트에 "목차 제목을 출력하지 말라"는 주의사항 추가
        chasm_prompt = (
            "보고서 섹션 '1.1 글로벌 전기차 시장 캐즘 현황'을 작성하세요.\n"
            "요구사항: 성장률 둔화/정의/원가 압력/중국 vs 비중국 관점 포함. 8~12줄.\n"
            "주의사항: 응답 텍스트에 '### 1.1...' 같은 목차 제목은 절대 포함하지 말고 본문 내용만 바로 작성하세요.\n"
            f"시장 RAG 근거:\n{market_rag}\n"
        )
        paradigm_prompt = (
            "보고서 섹션 '1.2 배터리 산업 패러다임 변화'를 작성하세요.\n"
            "요구사항: EV→HEV/ESS 등 수요 축 이동, 원가/규제/공급망, 기술 트렌드(예: LFP, 나트륨이온) 언급. 8~12줄.\n"
            "주의사항: 응답 텍스트에 목차 제목은 절대 포함하지 말고 본문 내용만 바로 작성하세요.\n"
            f"시장 RAG 근거:\n{market_rag}\n"
        )
        hev_pivot_prompt = (
            "보고서 섹션 '1.3 HEV 피벗과 완성차 전략 변화'를 작성하세요.\n"
            "요구사항: 전기차 캐즘에 따른 완성차 업체들의 하이브리드(HEV) 중심 전략 선회와 이것이 배터리 시장에 미치는 영향을 작성하세요. 8~12줄.\n"
            "주의사항: 응답 텍스트에 목차 제목은 절대 포함하지 말고 본문 내용만 바로 작성하세요.\n"
            f"시장 RAG 근거:\n{market_rag}\n"
        )
        outlook_prompt = (
        ...
        hev_pivot_text = invoke_llm(settings=self.settings, prompt=hev_pivot_prompt)
        ...
        return {
            ...
            "market_hev_pivot": hev_pivot_text,
